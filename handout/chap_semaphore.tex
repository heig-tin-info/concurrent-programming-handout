...
\par
Ainsi, si un verrou \ccode{v} est initialisé à vrai et qu'une tâche appelle \ccode{lock}, le verrou est positionné à faux. Si une seconde tâche appelle \ccode{lock}, alors cette tâche, qui est à l'état élu, passe à l'état bloqué et joint la file d'attente associée à \ccode{v}.
La primitive \ccode{lock} réalise l'opération inverse. S'il y a une tâche en attente sur le verrou \ccode{v}, alors cette tâche est réactivée. La tâche réveillée sort de la file d'attente associée à \ccode{v} pour joindre la file des tâches prêtes. Remarquons que l'exécution de la primitive \ccode{lock} revient à passer le verrou \ccode{v} à la tâche réveillée.
S'il n'y a pas de tâche en attente pour obtenir le verrou \ccode{v}, la primitive \ccode{lock} libère le verrou \ccode{v} en le positionnant à vrai (état initial).

\subsection{Section critique}
Comme son nom le sous-entend, un verrou permet de résoudre le problème de l'exclusion mutuelle à \ccode{n} tâches de manière simple. Il suffit de verrouiller un verrou \ccode{v} avant d'entrer en section, bloquant ainsi les autres tâches qui accèdent à leur section critique protégée par le même verrou \ccode{v}. La sortie d'une section critique se fait en déverrouillant \ccode{v}, ce qui libère le verrou ou réveille une tâche bloquée pour l'accès à sa section critique.
...
\begin{codeblock}[label=semaex:verrou,title=Implémentation de sémaphores par verrous]
typedef struct {
