...
Finalement, il est possible de développer en C ou C++. Pour un développement en C++, il faut travailler sur le fichier cppmain.cpp. Pour développer en C, il faut travailler dans cmain.c, et ajouter la ligne suivante dans le fichier \file{QtrainSim.pro}:
...
pour l'utilisation de la librairie. On remarque l'inclusion \ccode{#include "ctrain_handler.h"} qui fournit l'accès au fonctions. L'appel à \ccode{init_maquette()}
...
faut utiliser le langage C ou C++ (possibilité d'utiliser la librairie Qt). Toutefois, le projet est structuré de telle manière
que l'utilisation la plus simple consiste en la complétion
en C de la méthode run() utilisée par le thread \textit{User} dans
\file{main.cpp}. En aucun cas il n'est nécessaire, ni même
...
\item \submenu{Actions}{Emergency stop} fait appelle à la fonction \emph{emergency\_stop()} située à l'intérieur du fichier \texttt{cmain.c}. C'est la responsabilité de l'utilisateur de placer le code spécifique à cet appel.
...
\begin{figure}[!ht]
\includegraphics[width=.48\textwidth]{\string"fig/Maquette A1\string"}\includegraphics[width=.48\textwidth]{\string"fig/Maquette A2\string"}

\includegraphics[width=.48\textwidth]{\string"fig/Maquette B1\string"}\includegraphics[width=.48\textwidth]{\string"fig/Maquette B2\string"}
