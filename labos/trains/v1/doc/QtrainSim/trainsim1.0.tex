...
\hline
28/03/18 & v1.10 & YTA & Ajout d'un mot sur le déploiement \tabularnewline
\hline
\end{tabular}
\par\end{centering}
...
\attention Attention, avec \noun{QtCreator}, la compilation peut se faire en \emph{shadow build} ou non. Par défaut c'est le mode \emph{shadow build} qui est utilisé. Dans ce cas, il faut ajouter une étape de déploiement dans le panel \emph{Projects}, sous l'onglet \emph{Run}.
...
\begin{center}
	\includegraphics[width=.7\textwidth]{fig/deploy}
\end{center} 
...
De cette manière tous les fichiers maquettes sont copiés dans le répertoire de destination. L'autre option consiste à décocher l'option \emph{shadow build} dans l'onglet \emph{Build} du panel \emph{Projects}.
